...
// Concrete C
struct ConcreteDecoratorC {
    component: Rc<dyn Component>,
}

impl Decorator for ConcreteDecoratorC {
    fn new(component: Rc<dyn Component>) -> Self {
        ConcreteDecoratorC { component }
    }
}

impl Component for ConcreteDecoratorC {
    fn operation(&self) -> String {
        format!("ConcreteDecoratorC: {}", self.component.operation())
    }
}
...
fn main() {
    ...
    let decorator_a3 = ConcreteDecoratorC::new(Rc::new(decorator_a2));
    Client::client_code(&decorator_a3);
}
